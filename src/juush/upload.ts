    ...
    reqx.req.on("data", function(data){
        ...
        if(!headers){
            ...
            U.query.users.findOne({key: headers.key}).then(item => {
                ...
                const hostConfig = serverConst.juushHostMap?.[reqx.req.headers.host];
                if(hostConfig) {
                    customURL = hostConfig.url;
                    customURLSettings = { no_f: hostConfig.no_f };
                } else {
                    // Default: strip to root domain, prepend "i." prefix, no /f/
                    const hostParts = reqx.req.headers.host.split(".");
                    customURL = `i.${hostParts.slice(-2).join(".")}`;
